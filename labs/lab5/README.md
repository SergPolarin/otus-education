...
Команды проверки настроек протокола
    ...
    show ip protocols
    обеспечивает быструю проверку критически важных данных конфигурации OSPF.Выводит Process ID, Router ID,сети, объявляемые маршрутизатором, соседние устройства, от которых маршрутизатор получает обновления, и административное расстояние по умолчанию
    
    show ip ospf
    можно также использовать для проверки идентификатора процесса и идентификатора маршрутизатора в протоколе OSPFv2
    
    show ip ospf interface (brief) 
    команда выводит подробный список для каждого интерфейса, где включен OSPF. Для просмотра сводных данных по интерфейсам с включенным протоколом OSPFv2 использовать команду show ip ospf interface brief
